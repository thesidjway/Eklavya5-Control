...
//	cout<< "??????????????" << endl;
	...
	int reverse_button=joy->buttons[9];
	int brake_button=joy->buttons[10];
...
//		std::cout<< "---------------" <<std::endl;
    ...
    if(reverse_button==1 && reverse_data==1)
	{
		reverse_data=-1;
	}
	else if (reverse_button==1 && reverse_data==-1)
	{
		reverse_data=1;
	}

	if(brake_button==1 && brake_data==-1)
	{
		brake_data=1;
	}
	else if(brake_button==1 && brake_data==1)
	{
		brake_data=-1;
	}
...
void ModeSwitcher::planCallback(const geometry_msgs::Twist::ConstPtr& pose)
{
 ...
 void ModeSwitcher::publish(int argc, char** argv){
	...
	ros::Publisher send_brake = nh_.advertise<std_msgs::Int32>("brake_val", 5);
	ros::Publisher send_reverse = nh_.advertise<std_msgs::Int32>("reverse_val", 5);
	...
	while(ros::ok)
	{
		...
		std_msgs::Int32 revpub=brake_data;
		std_msgs::Int32 brakepub=reverse_data;
		if(temp_flag==1)
		{
			...
			//	cout;
				...
				brake_val.publish(brake_data);
				reverse_val.publish(reverse_data);
		}
